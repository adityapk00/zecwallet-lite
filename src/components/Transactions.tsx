...
const TxModalInternal: React.FC<RouteComponentProps & TxModalInternalProps>  = ({ modalIsOpen, tx, closeModal, currencyName, setSendTo, history }) => {
  let txid = '';
  let type = '';
  let typeIcon = '';
  let typeColor = '';
  ...
  const totalAmounts = tx && tx.detailedTxns ? tx.detailedTxns.reduce((s, t) => Math.abs(parseFloat(t.amount)) + s, 0) : 0;
